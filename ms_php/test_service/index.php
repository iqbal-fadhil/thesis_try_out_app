...
// Global handler untuk preflight
...
$auth_base_url = "http://localhost:8003"; // atau "http://157.15.125.7:8010"
...
function get_user_from_token($token, $auth_base_url) {
    ...
    if ($resp === false) {
        error_log("Auth service call failed for token=$token");
        return null;
    }

    $data = json_decode($resp, true);
    if (!is_array($data) || isset($data["error"])) {
        error_log("Auth service returned error for token=$token: " . $resp);
        return null;
    }
...
if ($method === "POST" && count($parts) === 1 && $parts[0] === "questions") {
    ...
    try {
        ...
        error_log("Insert question failed: " . $e->getMessage());
...
//         }
//     } catch (Exception $e) {
//         error_log("Question lookup failed: " . $e->getMessage());
...
//     // SEMENTARA: TIDAK DISIMPAN KE DB, HANYA RETURN RESULT
//     json_response([
//         "username"        => $username,
//         "submission_id"   => null,
//         "total_questions" => $total_questions,
//         "correct_answers" => $correct_answers,
//         "score"           => $correct_answers,   // biar React bisa baca data.score
//         "answers"         => $answers_result,
//     ]);
// }
...
if ($method === "POST" && count($parts) === 1 && $parts[0] === "submit") {
    ...
    // ============================
    // SIMPAN KE DB (tapi jangan bikin 500 kalau gagal)
    // ============================
    $submission_id = null;
    ...
    try {
        ...
        foreach ($answers_result as $ar) {
            $stmt_ans->execute([
                ...
                ":qid" => $ar["question_id"],
                ":sel" => $ar["selected_option"],
                ":isc" => $ar["is_correct"] ? 1 : 0,   // ← FIX di sini
            ]);
        }
        ...
        // Catat di log, tapi JANGAN kirim 500 ke client
        error_log("Submission save failed: " . $e->getMessage());
        // biarkan $submission_id = null
    }
    ...
    // json_response([
    //     "username"        => $username,
    //     "submission_id"   => $submission_id,              // kalau null berarti gagal simpan
    //     "total_questions" => $total_questions,
    //     "correct_answers" => $correct_answers,
    //     "score"           => $correct_answers,            // agar React bisa baca data.score
    //     "score"            => $score_percent,
    //     "answers"         => $answers_result,
    // ]);
